...
/*----------------------------------------
  page fade
------------------------------------------*/
const pageFade = document.querySelector("#page_fade");
...
// setTimeoutのdelayはbaseのpageFade::afterのtransitionと合わせる
  ...
  linkEl.addEventListener(
    ...
    (e) => {
      ...
      if (url !== "" && url.indexOf(currentHostName)) {
        if (pageFade) {
          pageFade.classList.remove("open");
        }
        setTimeout(() => {
          window.location = url;
        }, 600);
...
// SafariでブラウザバックするとJSなどが解除されていない問題【bfcache】の対策強制リロード
...
function ImageToYoutubeReplace(ytImage) {
  ...
  let youtubeVideo =
    '<iframe class="youtube_player" youtubeid="' +
    youtubeId +
    '" src="https://www.youtube.com/embed/' +
    youtubeId +
    '?playsinline=1&enablejsapi=1&rel=0" frameborder="0" allowfullscreen></iframe>';
...
const setFillHeightWidth = () => {
  ...
  if (inPageTitleArea) {
    ...
    document.documentElement.style.setProperty(
      "--in_page_title_area_height",
      `${inPageTitleAreaHeight}px`
    );
  }
};
...
window.addEventListener("load", () => {
  let globalVw = window.innerWidth;
});
...
/*-- fvHiddenElements --*/
const fvHiddenElements = document.querySelectorAll(".fv_scroll_hide");
window.addEventListener("scroll", function () {
...
const headerLogoScrollResizeElements = document.querySelectorAll(
  ".header_logo_scroll_resize"
);
window.addEventListener("scroll", function () {
  let scrollAmount = window.scrollY;
  if (scrollAmount > 0) {
...
const accordionSlideUp = (el) => {
  let h = el.offsetHeight;
  el.style.height = h + "px";
  el.animate([{ height: h + "px" }, { height: 0 }], {
    duration: 400,
    easing: "cubic-bezier(.63,.08,.47,.99)",
  });
  el.style.height = 0;
...
const accordionBoxes = document.querySelectorAll(".cmn_accordion_box");
accordionBoxes.forEach((accordion) => {
  const accordionTriggers = accordion.querySelectorAll(
    ".cmn_accordion_trigger"
  );
  const accordionContents = accordion.querySelectorAll(
...
/*----------------------------------------
  masonly
------------------------------------------*/
if (document.querySelector("#masonly")) {
  Macy({
    container: "#masonly",
    trueOrder: false,
    waitForImages: true,
    ...
    margin: 20,
    breakAt: {
      1200: { columns: 3, margin: 12 },
      856: { columns: 2, margin: 12 },
    },
  });
}
...
------------------------------------------*/
//---------- works swiper --------------
function initializeSwiperWorks(className) {
  let sliderSet = {
    slidesPerView: 1,
    centeredSlides: true,
    loop: false,
    ...
    breakpoints: {
      835: {
        spaceBetween: window.innerWidth * 0.01 * -12,
      },
    },
  };
  if (
    document.querySelectorAll(`.${className}_swiper .swiper-slide`).length == 1
  ) {
    sliderSet = {
      loop: false,
      pagination: false,
    ...
    document
      .querySelector(`.${className}_swiper`)
      .classList.add("no_controller");
  }
  const sliderVideos = new Swiper(`.${className}_swiper`, sliderSet);
}
initializeSwiperWorks("works");
...
/*-- mono modal --*/
/*-- separate modal --*/
const SeparateMonoModalOpens = document.querySelectorAll(
  ".separate_mono_modal_open_box .separate_mono_modal_open"
);
...
SeparateMonoModalOpens.forEach((SeparateMonoModalOpen) => {
  SeparateMonoModalOpen.addEventListener("click", () => {
    let SeparateMonoModalOpenIndex = SeparateMonoModalOpensArray.indexOf(
      SeparateMonoModalOpen
    );
    SeparateMonoModalBox.children[SeparateMonoModalOpenIndex].classList.add(
      "visible"
    );
  });
});
...
ytMonoModalOpens.forEach((ytMonoModalOpen) => {
  ytMonoModalOpen.addEventListener("click", () => {
    ...
    const ytMonoModalOpenTargetYtVideoBox =
      ytMonoModalOpenTarget.children[1].children[0];
    ImageToYoutubeReplace(ytMonoModalOpenTargetYtVideoBox);
  });
});
/*-- youtube modal --*/
/*-- youtube separate modal --*/
const ytSeparateMonoModalOpens = document.querySelectorAll(
  ".yt_separate_mono_modal_open_box .yt_separate_mono_modal_open"
);
const ytSeparateMonoModalBox = document.querySelector(
  ".yt_separate_mono_modal_box"
);
ytSeparateMonoModalOpens.forEach((ytSeparateMonoModalOpen) => {
  ytSeparateMonoModalOpen.addEventListener("click", () => {
    ...
    const ytSeparateMonoModalOpenTarget = ytSeparateMonoModalBox.querySelector(
      `[data-youtubeid="${youtubeId}"]`
    );
    if (ytSeparateMonoModalOpenTarget) {
      ytSeparateMonoModalOpenTarget.classList.add("visible");
      const ytSeparateMonoModalOpenTargetYtVideoBox =
        ytSeparateMonoModalOpenTarget.querySelector(".yt_video_box");
...
slideModals.forEach((slideModal) => {
  ...
  slideModalOpens.forEach((slideModalOpen) => {
    slideModalOpen.addEventListener("click", () => {
      ...
      slideModalContents.forEach((slideModalContent) => {
        slideModalContent.classList.remove("slide_visible");
      });
  ...
  function slideModalMoveRight() {
    if (slideModal.querySelector(".slide_visible")) {
      ...
      if (visibleContentIndex == slideModalContentsLength - 1) {
        ...
        slideModalContents[
          visibleContentIndex
        ].nextElementSibling.classList.add("slide_visible");
      }
    }
  }
  function slideModalMoveLeft() {
    if (slideModal.querySelector(".slide_visible")) {
      ...
      if (visibleContentIndex == 0) {
        slideModalContents[slideModalContentsLength - 1].classList.add(
          "slide_visible"
        );
      } else {
        slideModalContents[
          visibleContentIndex
        ].previousElementSibling.classList.add("slide_visible");
      }
    }
  }
...
modalCloseElements.forEach((modalCloseElement) => {
  modalCloseElement.addEventListener("click", () => {
    ...
    const slideModalContents = document.querySelectorAll(
      ".slide_modal_content"
    );
    slideModalContents.forEach((slideModalContent) => {
      slideModalContent.classList.remove("slide_visible");
...
document.addEventListener("DOMContentLoaded", function () {
  var characterItems = document.querySelectorAll(".character_item");
  characterItems.forEach(function (item) {
    item.addEventListener("click", function () {
      var index1 = Array.prototype.indexOf.call(characterItems, this);

      characterItems.forEach(function (ci) {
        ci.classList.remove("active");
      });
      this.classList.add("active");

      var parentElement = this.parentElement.parentElement;
      var characterList = parentElement.nextElementSibling;

      if (characterList && characterList.classList.contains("character_list")) {
        var characterContents =
          characterList.querySelectorAll(".character_content");
        characterContents.forEach(function (cc) {
          cc.classList.remove("active");
        });
        characterContents[index1].classList.add("active");
      }
    });
  });
});
